class Parser:
            ...
            if len(l) == 3:
                if l[2].isdigit():
                    if int (l[2])>= 0:
                        ...
                        self._flag = False
                        Parser._error ("Parser", n, "Cannot have negative location");
                        return ""

                else:
                    
                    self._flag = False
                    Parser._error ("Parser", n, "Wrong location");
                    return ""
            else:
                self._flag = False
    ...
    def _label(self, lab, n):
        ...
        if lab in y:
            ...
            c = self._dict.keys()
            self._flag = False
            Parser._error( x, "the goto/if-goto label ", c, "is wrong!");
            return ""
    
    def _goto(self, lab, n):
        self._dict.update({lab:n})
        return "@" + self._func + "$" + lab + "\n0;JMP"
